<?php
    ...
    Class CampeonatoInternacional extends Model 
    {

        const ID = 11; // ID do Campeonato Internacional
        ...
        public function getCompetitors() // Retorna o id de quais serão os participantes da competição
        {
            ...
            $teams = $sql->select( "SELECT id, rating FROM tb_teams WHERE country = :COUNTRYA OR country = :COUNTRYB OR country = :COUNTRYC OR country = :COUNTRYD", [
                ":COUNTRYA" => 'Paraguai',   
                ":COUNTRYB" => 'Chile',   
                ":COUNTRYC" => 'Colômbia',   
                ":COUNTRYD" => 'Uruguai',   
        ...
        public function saveCompetitors() // Salva os competidores do campeonato
        {
            ...
            for ( $i = 0; $i < count( $teams ); $i++ ) {
                ...
                $add = " (" . Season::getCurrent() . ", " . CampeonatoInternacional::ID . ", " . 1 . ", " . (int) $teams[$i]['id'] . "),";
        ...
        public function saveMatchdays() // Salva quais serão as rodadas do campeonato
        {
            ...
            $competition = CampeonatoInternacional::ID;
            $nrgroup = 1;
            ...
            for ($i = 0; $i < count( $matchdays ) ; $i++) { 
                ...
                for ($x = 0; $x < count( $matchdays[$i] ) ; $x++) { 
                    ...
                    if ( in_array( $round, CampeonatoInternacional::MIDDLEWEEK_MATCHES ) ) {

                        $randkey = array_rand( CampeonatoInternacional::MIDDLEWEEK_TIMES );

                        $matchtime = CampeonatoInternacional::MIDDLEWEEK_TIMES[$randkey];

                    } else {

                        $randkey = array_rand( CampeonatoInternacional::WEEKEND_TIMES );

                        $matchtime = CampeonatoInternacional::WEEKEND_TIMES[$randkey];
        ...
        public function scoreboard() // Salva os resultados das partidas
        {
            ...
            for ( $i = 0; $i < count( $teams ); $i++ ) {
                ...
                LeagueFunctions::updateStanding( $teams[$i]['id'], CampeonatoInternacional::ID, 1, 1);
        ...
        public function listStandings() // Traz a tabela de classificação
        {
            ...
            for ( $i = 0; $i < count( $standings ); $i++ ) { 
                ...
                $standings[$i]['lastResults'] = LeagueFunctions::getTeamLastResults( $standings[$i]['id'], CampeonatoInternacional::ID, 1 );
        ...
        public function load()
        {
            ...
            $page->render('stage', [
                ...
                'matches' => [
                    ...
                    'saveURL' => '/campeonato-internacional',
                ]
            ]);
