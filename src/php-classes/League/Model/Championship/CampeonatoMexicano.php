<?php
    ...
    Class CampeonatoMexicano extends Model 
    {

        const ID = 15; // ID do Campeonato Mexicano
        ...
        public function saveCompetitors() // Salva os competidores do campeonato
        {
            ...
            for ( $i = 0; $i < count( $teams ); $i++ ) {
                ...
                $add = " (" . Season::getCurrent() . ", " . CampeonatoMexicano::ID . ", " . 1 . ", " . (int) $teams[$i]['id'] . "),";
        ...
        public function saveMatchdays() // Salva quais serão as rodadas do campeonato
        {
            ...
            $competition = CampeonatoMexicano::ID;
            $nrgroup = 1;
            ...
            for ($i = 0; $i < count( $matchdays ) ; $i++) { 
                ...
                for ($x = 0; $x < count( $matchdays[$i] ) ; $x++) { 
                    ...
                    if ( in_array( $round, CampeonatoMexicano::MIDDLEWEEK_MATCHES ) ) {

                        $randkey = array_rand( CampeonatoMexicano::MIDDLEWEEK_TIMES );

                        $matchtime = CampeonatoMexicano::MIDDLEWEEK_TIMES[$randkey];

                    } else {

                        $randkey = array_rand( CampeonatoMexicano::WEEKEND_TIMES );

                        $matchtime = CampeonatoMexicano::WEEKEND_TIMES[$randkey];
        ...
        public function scoreboard() // Salva os resultados das partidas
        {
            ...
            for ( $i = 0; $i < count( $teams ); $i++ ) {
                ...
                LeagueFunctions::updateStanding( $teams[$i]['id'], CampeonatoMexicano::ID, 1, 1);
        ...
        public function listStandings() // Traz a tabela de classificação
        {
            ...
            for ( $i = 0; $i < count( $standings ); $i++ ) { 
                ...
                $standings[$i]['lastResults'] = LeagueFunctions::getTeamLastResults( $standings[$i]['id'], CampeonatoMexicano::ID, 1 );
